...
function projectBehaviorScore(context) {
  const patterns = [
    /\bawait\b/,
    /\bcatch\b/,
    /\bfor\b/,
  ...
  return patterns.reduce(
    (score, pattern) => score + (pattern.test(context) ? 1 : 0),
    0,
  );
}
  ...
  for (const filePath of files) {
    ...
    lines.forEach((line, index) => {
      ...
      if (score > 0) {
        ...
        const semanticContext = buildProjectSemanticContext(lines, index);
        matches.push({
          path: rel,
          line: index + 1,
          ...
          semanticContext,
          behaviorScore: projectBehaviorScore(semanticContext),
          kind: structure.kind,
