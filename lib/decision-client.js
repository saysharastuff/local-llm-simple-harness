...
export function getDecisionConfig(env = process.env) {
  ...
  return {
    ...
    trace:\n      (env.HARNESS_DECISION_TRACE ?? "false").trim().toLowerCase() === "true",
  };
}
...
export async function requestDecision({
  ...
  try {
    ...
    if (!Array.isArray(payload?.ranking) || payload.ranking.length === 0) {
      throw new Error(\n        "Decision Engine response is missing a non-empty ranking.",\n      );
    }
    ...
    if (
      ...
      throw new Error(\n        `Decision Engine request timed out after ${timeoutMs} ms.`,\n      );
    }
    throw err;
  } finally {
